...
CREATE TABLE IF NOT EXISTS rag_decision_log (
    ...
    rag_mode            VARCHAR(16) NOT NULL COMMENT '意图路由预判: KB/GENERAL/TOOL(P3-7 起仅作诊断参考, 作答口径看 answer_outcome)',
    answer_outcome      VARCHAR(32) NULL COMMENT '本轮实际出口: ANSWERED_FROM_KB/ANSWERED_FROM_CACHE/REFUSED_NO_EVIDENCE/ANSWERED_OPEN/TOOL_DATA(事后由检索事实算出, 非事前预判)',
    session_type        VARCHAR(20) NOT NULL COMMENT '会话类型: RAG/AGENT/HYBRID',
    retrieval_executed  BOOLEAN     NOT NULL DEFAULT FALSE COMMENT '是否真的执行了检索(工具轮与 AGENT 会话为 0); 缓存命中自 P3-7 起由 answer_outcome=ANSWERED_FROM_CACHE 标识, 不再由本列反推',
