 ...
 * 或拒绝(生产基线已在 application-prod.yaml 设为 false, 并由 {@code SecurityConfigValidator}
 * 强制——prod 若配成 true 直接拒绝启动)。
 *
 * <p>降级可观测性: 异常一律 WARN(禁止 DEBUG)并经 {@link WarnThrottle} 折成 60 秒一条——
 * 鉴权路径每请求都查黑名单, 不节流会刷屏。
 */
@Slf4j
@Component
...
public class TokenBlacklistService {
    ...
    public void ban(String jti, long ttlMs) {
        ...
        try {
            ...
            degraded.warn("令牌拉黑失败(已降级: 该令牌注销未生效): {}", e.getMessage());
        }
    }
    ...
    public boolean isBanned(String jti) {
        ...
        try {
            ...
            degraded.warn("令牌黑名单检查失败(本次已{}): {}",
                    failOpen ? "按放行处理" : "按拒绝处理", e.getMessage());
            return !failOpen;
        }
    }
}
